...
func get_input(delta : float) -> void:
	...
	# Small dash
		...
		if Input.is_action_just_pressed("keyboard_up"): small_dash_dir.y = -1
		if Input.is_action_just_pressed("keyboard_down"): small_dash_dir.y = 1
		if Input.is_action_just_pressed("keyboard_right"): small_dash_dir.x = 1
		if Input.is_action_just_pressed("keyboard_left"): small_dash_dir.x = -1
		
		if (small_dash_dir != Vector2.ZERO):
			velocity = small_dash_dir * DASH_SPEED/4
			$DirectionDashTimer.start()
			DASHING = true
	...
	if Input.is_action_just_pressed("mouse_left"): 
		velocity = (mouse_pos - player_pos).normalized() * DASH_SPEED/2
